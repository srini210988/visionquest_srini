...
import { ChevronLeft, ChevronRight,ALargeSmall,Play, Pause, Volume2, VolumeX } from 'lucide-react'
...
export default function VideoDetail1() { 
...
console.log("fetchData");
  ...
  //  setCurrentId(queryParam);
    ...
    // Get current slide from query parameter, default to first slide
  const currentSlideParam = searchParams.get('slide')
  const initialSlide = currentSlideParam 
    ? parseInt(currentSlideParam) 
    : 1
  ...
  const goToSlide = (slideId) => {
    // Update URL with new slide parameter
  ...
  const prevSlide = () => {
    const prevSlideId = currentSlide > 1 
      ? currentSlide - 1 
      : IMAGES.length
  ...
  // Next slide handler
  const nextSlide = () => {
    const nextSlideId = currentSlide < IMAGES.length 
      ? currentSlide + 1 
      : 1
